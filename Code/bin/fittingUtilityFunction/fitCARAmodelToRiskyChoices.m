function [rho,sigma,CI95] = fitCARAmodelToRiskyChoices(valRiskyHigh,valRiskyLow,valSafe,chooseRisky,plotFlag,figNameStr)
% function [rho,sigma,CI95] = fitCARAmodelToRiskyChoices(valRiskyHigh,valRiskyLow,valSafe,chooseRisky,plotFlag,figNameStr)
%
...
Link_probit_AiO = @(cRiskyHigh, cRiskyLow, cSafe, rho, sigma) ...
  Link_probit(CE_FCT_AiO(cRiskyHigh, cRiskyLow, rho), cSafe, sigma);
...
if demo % create template subjects performing a set of decisions
  ...
  if useCRRA
    ...
    rho       = [-0.5            0              0.5]'; % !! for rho = 0.5, I get crazy certainty equivalents
  end
  ...
  for s = 1:length(subjNames)
    chooseRisky{s} = Link_probit_AiO(simDatRiskyHigh, simDatRiskyLow, simDatValSafe, T.rho(s), sigma) > 0.5;  % No noise: step function
    chooseRisky{s} = Link_probit_AiO(simDatRiskyHigh, simDatRiskyLow, simDatValSafe + randn(Nt,1) * sigma, T.rho(s), sigma)  > 0.5;
...
%     end
  ...
  if plotFlag
    ...
    plot(x_sort,chooseRisky_fit(order),'-','linewidth',2);
    plot([0 0],[0 1],[min(x) max(x)],[.5 .5],'-k')
    grid on
    ylabel('P(lottery)')
    xlabel('CE(lottery) - CE(safe)')
    legend({'Orig data','Fitted data'},'location','NorthWest')
    title(sprintf('Rho: %.3f; sigma: %.3f; SSresid: %.2f',rho,sigma,SSresid))
  end % plot
