function [result] = fit_happy_model_responsibility(mtx,happyscore,constant)
...
result.modelName = 'Responsibility';
...
result.paramNames = {'CR','EV','sRPE','gamma','social_pRPE','partner_pRPE'};
if exist('constant','var') && ~isnan(constant)
...
try s1 = x(5);  %partner RPE resulting from self decisions, regressor called "self"
  s2 = x(6);  %partner RPE resulting from partner decisions, regressor called "partner"
end
if length(x)==7,
    const = x(7);
else
