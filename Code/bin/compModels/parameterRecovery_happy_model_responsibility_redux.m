function [recov_params,results] = parameterRecovery_happy_model_responsibility_redux(mtx,params,noiseLevel,Nrep,displayFlag)
% mtx are input regressors
%assumes that each row of the cell array is a different subject

%% Settings
constant = NaN;   % has always been like that
rptfit = 5;       % repeat N times jittering the betas (as during fitting)
% noiseLevel = .2;     % intensity of noise to add to generated data; units = SD happiness
% Nrep = 50;        % how many times the parameter recovery is repeated
% displayFlag = 1;  % if 1, makes figure

%% Get needed variables
tempf=fieldnames(mtx);
for n=1:length(tempf), eval(sprintf('mtx.%s=double(mtx.%s);',tempf{n},tempf{n})); end;

%% Generate fake data based on input regressors and parameters to recover, add noise, add see if the recovered parameters look like the input ones
dataToFit = []; % collect data to fit
for m = 1:Nrep % repeat Nrep times
  
  % Make the data
  mtx.happyscore = NaN(size(mtx(1).certainmtx,1),1);
  [~, happysynth] = model_param(params, mtx);
  
  % Then, add some noise, in units of standard deviations of the generated data
  happysynth = happysynth + randn(size(happysynth)) * std(happysynth) * noiseLevel;
  dataToFit(m,:) = happysynth;
  mtx.happyscore = double(happysynth);
  
  % Now fit fake data and check if I recover the parameters used to generate the data
  if isfield(mtx,'inx')
    inx = mtx.inx;
  else
  ...
  result.b = b;
  [lse, happypred, r2, r2adj] = model_param(b, mtx);
  
  for n=1:rptfit, %jitter the betas
    b = fmincon(@model_param, double(b+0.1*randn(size(b))), [],[],[],[],lb,ub,[], options, mtx);
    lse2 = model_param(b, mtx);
    if lse2<lse,
      result.b = b;
      [lse, happypred, r2, r2adj] = model_param(b, mtx);
    end;
...
%% Collect recovered parameters
recov_params = vertcat(results(:).b);

%% Display if desired
if displayFlag
  wide_figure; 
  subplot(1,5,1:4); plot(dataToFit'); title('data to fit')
  subplot(1,5,5)
  bar(params); hold on; violinDots(1:5,recov_params,.3);
  xlabel_oblique(result.paramNames)%,'interpreter','none')
end
