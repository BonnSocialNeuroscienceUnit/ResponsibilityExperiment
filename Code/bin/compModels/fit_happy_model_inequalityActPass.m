function [result] = fit_happy_model_inequalityActPass(mtx,happyscore,constant)
...
if isfield(mtx,'inx'),
    ...
    inx = [0.5 0.5 0.5 0.5    0 0]; %dummies 
end
...
result.paramNames = {'CR','EV','sRPE','decay','inequalAct','inequalPass'};
if exist('constant','var') && ~isnan(constant)
...
guiltenvyActive  = (ywtl+yltw).*repmat(data.conds==1,1,size(happypred,1))';
guiltenvyPassive = (ywtl+yltw).*repmat(data.conds==2,1,size(happypred,1))';
%yltl = data.notenvymtx;
% HERE THE MODEL IS DEFINED
if exist('s1','var')
%   happypred = a*cert*dec + b*ev*dec + c*rpe*dec + ...
%     s1*ywtl*dec + s2*yltw*dec + const;
  happypred = a*cert*dec + b*ev*dec + c*rpe*dec + ...
    s1*guiltenvyActive*dec + s2*guiltenvyPassive*dec + const;
else
  happypred = a*cert*dec + b*ev*dec + c*rpe*dec + ...
    const;
end
