function [result] = fit_happy_model_otherRPEactPass(mtx,happyscore,constant)
...
result.paramNames = {'certain','ev','rpe','decay','otherRPEact','otherRPEpass'};
if exist('constant','var') && ~isnan(constant)
...
try s1 = x(5);  %partner RPE resulting from self decisions
  s2 = x(6);  %partner RPE resulting from partner decisions
end
if length(x)==7,
    const = x(7);
else
...
if exist('s1','var')
  happypred = a*cert*dec + b*ev*dec + c*rpe*dec + ...
    s1*otherrpeactive*dec + s2*otherrpepassive*dec + const;
else
  happypred = a*cert*dec + b*ev*dec + c*rpe*dec + ...
    const;
end
